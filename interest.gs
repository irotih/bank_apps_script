/**
 * Get the current interest APR from Cell A8
 */
function getCurrentInterestRate() {
  const sheet = getLedger();
  return sheet.getRange(1, 8).getValue();
}
...
/**
 * Return the last interest as { date: date of interest payment, row: row number of interest row}
 */
